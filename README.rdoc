...
1. Order.update_status: Randomly select an unresolved order (an order that is 'open' or 'pending') and update its status if is has changed
...
2. Contract.update_status: Randomly select a contract that is awaiting a status update and update it
    ...
    - update contract's status to 'done'
...
5. Order.cancel_stale_orders: Cancel orders that have been sitting for awhile
    - cancel buy-orders if they were created more than 5 minutes ago and there have not been any
      buy-orders completed in the last 2 minutes
    - cancel sell-orders if they were created more than 5 minutes ago and there have not been any
      sell-orders completed in the last 2 minutes
...
7. Contract.place_new_sell_order: Place a new sell order
    ...
    - if there is not a backlog of sell orders (a backlog is 50 sell orders)
