      ...
      if waiting_for_entry?
        ...
        if contract.buy_orders.active.where(stop_type: nil).any?
          ...
          if buy_order.done?
            ...
            ##########   Place stop order   ##########
            stop_order_floor         = (buy_order.filled_price * (1.0 - STOP_ORDER_PERCENT)).round(2)
            ...
            if contract.sell_orders.active.stop_orders.any? # GDAX::Connection.new.rest_client.orders(status: 'open').select {|o| o.type == 'limit' && o.stop == 'entry' && o.side == 'sell' }.any?
              ...
              if current_stop_order_price > (active_stop_order.stop_price * (1 + 0.0005))
                ...
                if size == 0.0
                  ...
                  place_stop_sell(current_stop_order_price, current_stop_order_price, size, contract.id, algorithm) # NOTE: the stop_order_price is the market price that will trigger the order; the limit_price is the price that it will be sold for
                end
              end
            else
              return if contract.resolvable? # this handles an edge case where the stop order has filled and been updated to 'done' but the contract hasn't yet been updated
