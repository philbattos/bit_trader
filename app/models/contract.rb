class Contract < ActiveRecord::Base
  ...
  def self.match_open_buys
    ...
    if open_contracts.any?
      ...
      open_contracts.each do |contract|
        next unless Order::ACTIVE_STATUSES.include? contract.buy_order.status # NOTE: this is a temporary hack to avoid statuses that cause contract.buy_order to return nil. One long-term solution is to re-write with_buy_without_sell scope as SQL query
  ...
  def self.match_open_sells
    ...
    if open_contracts.any?
      ...
      open_contracts.each do |contract|
        next unless Order::ACTIVE_STATUSES.include? contract.sell_order.status # NOTE: this is a temporary hack. see above.
